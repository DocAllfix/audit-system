...
# Versione: 3.1 — v3 spike (alleggerito tier MINIMO + categorie ASCII + regole YAML)
...
**Esempio di scheda corretta (copia questo schema):**
...
- **HEADER COMPATTO OBBLIGATORIO — 4 campi minimi**: `tipo`, `categoria`, `titolo`, `data_doc`. Questi 4 campi sono il routing minimo che permette di non perdere il documento nei sistemi a valle. Devono SEMPRE essere presenti.
- **Header esteso opzionale (raccomandato se i dati ci sono)**: `riferimento`, `data_scadenza`, `emesso_da`, `soggetto`, `firme`. Aggiungili quando il documento li riporta. Se assenti, ometti il campo (NON scrivere `n.d.` per questi campi opzionali nel tier MINIMO — riduce rumore).
...
- Lunghezza tipica indicativa: 40-120 token per scheda (header compatto + cluster opzionale).
...
**Esempio scheda Tier MINIMO corretta (fattura):**
...
**Esempio scheda Tier MINIMO ridottissimo (attestato formazione, dati minimi):**

```yaml
# ── DOC 7 ──
tipo: "Attestato Formazione Sicurezza"
categoria: "10 - SALUTE E SICUREZZA SUL LAVORO"
titolo: "Attestato corso aggiornamento art. 37 D.Lgs 81/08"
data_doc: "08/02/2026"
```
...
| Header scheda | ✅ **Tier MINIMO**: 4 campi minimi (`tipo`, `categoria`, `titolo`, `data_doc`) + opzionali; **Tier MEDIO/ESTESO**: 9 campi completi (`tipo`, `categoria`, `titolo`, `riferimento`, `data_doc`, `data_scadenza`, `emesso_da`, `soggetto`, `firme`) |
...
**PARTE A — HEADER (compatto in tier MINIMO, completo in tier MEDIO/ESTESO)**
- **Tier MINIMO**: header compatto a 4 campi obbligatori (`tipo`, `categoria`, `titolo`, `data_doc`) + 5 opzionali se i dati ci sono (`riferimento`, `data_scadenza`, `emesso_da`, `soggetto`, `firme`). Tutti gli altri campi vanno omessi se assenti — non scrivere `n.d.` per ridurre rumore.
...
- Tier MINIMO: 0-1 cluster compatto
...
1. **Conteggio schede — UNA SCHEDA PER OGNI DOCUMENTO RICEVUTO** — il numero di blocchi `# ── DOC N ──` emessi DEVE essere ESATTAMENTE uguale al numero di documenti ricevuti in input.
   ...
   - Conta sempre prima di chiudere il fence finale. Se il conteggio non quadra, completa le schede mancanti come Tier MINIMO (4 campi minimi).
...
2. **Header per OGNI scheda** — verifica:
   - **Tier MINIMO**: ogni scheda ha almeno i 4 campi obbligatori (`tipo`, `categoria`, `titolo`, `data_doc`). Gli altri 5 sono opzionali (aggiungi se ci sono).
