...
## REGOLA PRELIMINARE (INDEROGABILE — DEEPSEEK ONLY)
...
- **HEADER COMPLETO OBBLIGATORIO** (gli stessi 9 campi del Tier MEDIO): `tipo`, `categoria`, `titolo`, `riferimento`, `data_doc`, `data_scadenza`, `emesso_da`, `soggetto`, `firme`. Questo è inderogabile: senza header completo (e in particolare senza `categoria`), il documento perde il routing e finisce fuori contesto. Per `data_scadenza` non applicabile usa `non applicabile`; per campi assenti usa `n.d.` (vedi Regola 2.3).
...
- Lunghezza tipica indicativa: 80-200 token per scheda (header completo + cluster compatto).
...
| Header scheda (9 campi) | ✅ **INDEROGABILE in TUTTI i tier**: `tipo`, `categoria`, `titolo`, `riferimento`, `data_doc`, `data_scadenza`, `emesso_da`, `soggetto`, `firme` |
...
**PARTE A — HEADER FISSO (9 campi, INDEROGABILE in TUTTI i tier — ESTESO, MEDIO, MINIMO)**
I 9 campi sotto sono SEMPRE presenti, sempre nello stesso ordine, sempre con questi nomi esatti. Mai rinominarli, mai trattarli come cluster. Per campi assenti usa `n.d.` (vedi Regola 2.3 — solo per i campi header, non per i cluster).
...
1. **Conteggio schede** — il numero di blocchi `# ── DOC N ──` emessi DEVE essere uguale al numero di documenti ricevuti in input. Se mancano, completale come Tier MINIMO (header completo + cluster compatto). **Mai omettere una scheda**: meglio una scheda ridotta che zero schede per un file.

2. **Header completo per OGNI scheda** — verifica che in ogni scheda siano presenti i 9 campi: `tipo`, `categoria`, `titolo`, `riferimento`, `data_doc`, `data_scadenza`, `emesso_da`, `soggetto`, `firme`. Mai trattare l'header come cluster libero. Per campi assenti usa `n.d.` (mai omettere il campo).
